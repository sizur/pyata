...
class UnificationIterables:
    ...
    """
    @classmethod
    def unify_hook(cls: type[Self], arg: tuple[Ctx, Any, Any]
                   ) -> tuple[Ctx, Any, Any]:
        ctx, x, y = arg
        if not ctx or x == y:
        ...
        try:                          # Try unify iterators iteratively.
            ...
            return arg  # Either one or both not iterable: don't handle.
        while True:
            try:
            ...
            ctx, x_i = Substitutions.walk(ctx, x_i)  # pyright: ignore[reportUnboundVariable]
            ctx, y_i = Substitutions.walk(ctx, y_i)  # pyright: ignore[reportUnboundVariable]
            if x_i is ...:
                try:
